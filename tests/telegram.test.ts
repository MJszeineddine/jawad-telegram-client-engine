...
function signed(token:string,auth:number){const p=new URLSearchParams({auth_date:String(auth),query_id:"q",user:JSON.stringify({id:123,first_name:"A"})});const data=[...p.entries()].sort(([a],[b])=>a.localeCompare(b)).map(([k,v])=>`${k}=${v}`).join("\n");const secret=createHmac("sha256","WebAppData").update(token).digest();p.set("hash",createHmac("sha256",secret).update(data).digest("hex"));return p.toString()}
test("validates Telegram Mini App init data",()=>{const now=1000;assert.equal(validateTelegramInitData(signed("token",now),"token",900,now).ok,true);assert.equal(validateTelegramInitData(signed("token",now),"wrong",900,now).reason,"INVALID_HASH");assert.equal(validateTelegramInitData(signed("token",1),"token",10,now).reason,"STALE_AUTH_DATE")});
