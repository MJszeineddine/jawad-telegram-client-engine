...
test("confirms a valid transfer and is idempotent",()=>{const {l,i}=setup();assert.equal(l.verifyAndAssign(i.id,tx(),12,now+2000).code,"PAYMENT_CONFIRMED");assert.equal(l.verifyAndAssign(i.id,tx(),12,now+2000).code,"ALREADY_PAID_IDEMPOTENT")});
