...
export class LocalAttachmentStore {
  ...
  private inside(path:string){return path===this.root||path.startsWith(`${this.root}${sep}`)}
  ...
  async save(input:{name:string;mime:string;bytes:Buffer;retentionDays?:number}):Promise<StoredAttachment>{
    ...
    await mkdir(this.root,{recursive:true,mode:0o700}); const id=randomUUID(); const storagePath=join(this.root,`${id}.bin`); if(!this.inside(resolve(storagePath)))throw new Error("INVALID_STORAGE_PATH"); await writeFile(storagePath,input.bytes,{mode:0o600});
  ...
  async read(record:StoredAttachment){const path=resolve(record.storagePath);if(!this.inside(path))throw new Error("OUTSIDE_ATTACHMENT_ROOT");return readFile(path)}
  async delete(record:StoredAttachment){const path=resolve(record.storagePath);if(!this.inside(path))throw new Error("OUTSIDE_ATTACHMENT_ROOT");await unlink(path)}
  async readStorageKey(storageKey:string){if(!/^[a-f0-9-]{36}\.bin$/i.test(storageKey))throw new Error("INVALID_STORAGE_KEY");const path=resolve(join(this.root,storageKey));if(!this.inside(path))throw new Error("OUTSIDE_ATTACHMENT_ROOT");return readFile(path)}
  async deleteStorageKey(storageKey:string){if(!/^[a-f0-9-]{36}\.bin$/i.test(storageKey))throw new Error("INVALID_STORAGE_KEY");const path=resolve(join(this.root,storageKey));if(!this.inside(path))throw new Error("OUTSIDE_ATTACHMENT_ROOT");try{await unlink(path)}catch(error){if((error as NodeJS.ErrnoException).code!=="ENOENT")throw error}}
}
