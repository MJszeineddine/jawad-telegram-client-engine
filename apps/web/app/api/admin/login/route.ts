...
export async function POST(request:Request){
  if(!sameOrigin(request.url,request.headers.get("origin")))return NextResponse.json({error:"ORIGIN_REJECTED"},{status:403});const limit=await allowRequest("admin-login",requestAddress(request));if(!limit.allowed)return NextResponse.json({error:"RATE_LIMITED",retryAfterMs:limit.retryAfterMs},{status:429});
  const form=await request.formData();const password=String(form.get("password")??"");const demo=process.env.NODE_ENV!=="production"&&(process.env.DEMO_MODE??"true")==="true";const expected=process.env.ADMIN_PASSWORD_SHA256;
