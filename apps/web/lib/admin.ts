...
export async function loadDashboard(): Promise<{ snapshot: DashboardSnapshot; source: "database" | "demo" | "degraded"; error?: string }> {
  if (isDemoMode()) return { snapshot: demoSnapshot(), source: "demo" };
  if (!process.env.DATABASE_URL || !process.env.DATA_ENCRYPTION_KEY) return { snapshot: { counts: {}, leads: [] }, source: "degraded", error: "Database or encryption configuration is missing." };
  ...
  catch (error) { return { snapshot: { counts: {}, leads: [] }, source: "degraded", error: error instanceof Error ? error.message : "Database unavailable" }; }
...
export async function loadOperations():Promise<{capacity:CapacitySettingsDetail;groups:GroupMonitorDetail[];source:"database"|"demo"|"degraded";error?:string}>{
  ...
  if(isDemoMode())return{capacity:demoCapacity,groups:[{groupId:"-100000000001",title:"Authorised demo group",adminAuthorised:true,enabled:true,minimumScore:6,keywordCategories:["developer_request","react_next","node_api","production_deployment","database_auth","urgent_today","unfinished_handoff"],responseMode:"notify_only",retentionDays:30,authorisedBy:"demo-admin",authorisedAt:new Date().toISOString()}],source:"demo"};
  if(!process.env.DATABASE_URL||!process.env.DATA_ENCRYPTION_KEY)return{capacity:{maximumActiveQuickFixes:0,maximumRescueJobs:0,pauseCheckout:true,workingHours:{},awayMode:true},groups:[],source:"degraded",error:"Database or encryption configuration is missing."};
  const repository=await createPostgresRepository(process.env.DATABASE_URL,process.env.DATA_ENCRYPTION_KEY);try{return{capacity:await repository.getCapacitySettings(),groups:await repository.listGroupMonitors(),source:"database"}}catch(error){return{capacity:{maximumActiveQuickFixes:0,maximumRescueJobs:0,pauseCheckout:true,workingHours:{},awayMode:true},groups:[],source:"degraded",error:error instanceof Error?error.message:"Operations settings unavailable"}}finally{await repository.close()}
}
...
export async function loadPartners():Promise<{partners:PartnerStatsDetail[];referrals:ReferralLedgerDetail[];source:"database"|"demo"|"degraded";error?:string}>{
  ...
  if(isDemoMode())return{partners,referrals,source:"demo"};
