...
export class MemoryBotWizardStore implements BotWizardStore {
  ...
  async set(key:string,state:BotWizardState,ttlSeconds=7200){this.entries.set(key,{state:structuredClone(state),expiresAt:Date.now()+ttlSeconds*1000})}
